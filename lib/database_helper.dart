...
class DatabaseHelper {
  ...
  Future<Database> _initiateDatabase() async {
    ...
    //       options: OpenDatabaseOptions(onCreate: _onCreate, version: _dbVersion));
    //   if (_database == null) print('데이터 베이스 초기화 null임');
    //   return _database!;
  }

  Future _onCreate(Database db, int version) async {
    //query 생성, table 이름, row, colum 설정해주는 것
    db.execute("CREATE TABLE IF NOT EXISTS myTable (name String, _id INTEGER)");
    List<Map<String, dynamic>> idd = [];
    print('hello');
    for (var i = 0; i < 5; i++) {
      idd.add({
        "columnName": "$i-start",
    ...
    for (var i = 0; i < idd.length; i++) {
      Map<String, dynamic> row = {
        columnName: idd[i]["columnName"],
        columnId: idd[i]["columnId"],
      };
  ...
  //{"_id":12, "name":"Soojin"} 이런 형태로 Map이니까
